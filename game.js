...
let state = {
  ...
  log: ["The Crown has granted your company a charter. One merchantman waits in London. Click a marked port, then Sail. Ships now travel in real time."],
  gameOver: false,
  paused: false,
  monthDurationMs: 30000,
...
function fleetSpeed() {
  if (!state.ships.length) return 1;
  const total = state.ships.reduce((sum, ship) => sum + shipTypes[ship.type].speed, 0);
  return total / state.ships.length;
}

function voyageDuration(fromId, toId) {
  const dist = portDistance(fromId, toId);
  return clamp(Math.round((dist / fleetSpeed()) * 650), 4500, 24000);
}

function shipPosition() {
  if (!state.voyage) return { x: currentPort().x, y: currentPort().y, moving: false };
  const from = ports[state.voyage.from];
  const to = ports[state.voyage.to];
  const progress = clamp((Date.now() - state.voyage.startedAt) / state.voyage.duration, 0, 1);
  return {
...
function renderMap() {
  ...
  Object.entries(ports).forEach(([id, port]) => {
    ...
    btn.title = port.name;
    btn.setAttribute("aria-label", port.name);
  ...
  state.rivals.forEach((rival, index) => {
    const p = ports[rival.location];
    const token = document.createElement("div");
    token.className = "rival-token";
    token.style.left = `${p.x + (index - 1) * 1.1}%`;
    token.style.top = `${p.y + 2.2}%`;
    token.title = rival.name;
    token.textContent = "⚑";
  ...
  ship.className = `ship-token ${pos.moving ? "moving" : ""}`;
...
function renderCompany() {
  ...
  const timeLeft = Math.max(0, Math.ceil((state.monthDurationMs - (Date.now() - state.monthStartedAt)) / 1000));
  document.getElementById("timeLabel").textContent = state.paused ? "Paused" : `Next month: ${timeLeft}s`;

  const locationText = state.voyage
    ? `At sea: ${ports[state.voyage.from].name} → ${ports[state.voyage.to].name}`
    : currentPort().name;
...
function renderActions() {
  const port = selectedPort();
  const here = state.selectedPort === state.location && !isAtSea();
  const buttons = [];
  const hint = document.getElementById("actionHint");

  if (isAtSea()) {
    const progress = clamp((Date.now() - state.voyage.startedAt) / state.voyage.duration, 0, 1);
    const left = Math.max(0, Math.ceil((state.voyage.duration - (Date.now() - state.voyage.startedAt)) / 1000));
    hint.innerHTML = `Fleet at sea: ${ports[state.voyage.from].name} → ${ports[state.voyage.to].name}. Arrival in about ${left}s.<div class="voyage-meter"><span style="width:${Math.round(progress * 100)}%"></span></div>`;
    buttons.push(actionButton(state.paused ? "Resume" : "Pause", "build", togglePause));
    actionsEl.innerHTML = "";
...
function sailTo(portId) {
  ...
  state.voyage = {
    ...
    to: portId,
    startedAt: Date.now(),
    duration: voyageDuration(from, portId),
    travelCost
  };
  state.selectedPort = portId;
  addLog(`Fleet departed ${ports[from].name} for ${ports[portId].name}. Voyage cost: ${money(travelCost)}.`);
  render();
}

function finishVoyage() {
  if (!state.voyage) return;
  const from = ports[state.voyage.from].name;
  const toId = state.voyage.to;
  state.location = toId;
  state.selectedPort = toId;
  state.voyage = null;
  damageFleet(randomInt(0, 8));
  addLog(`Arrived in ${ports[toId].name} after sailing from ${from}.`);
  randomVoyageEvent();
  render();
}
...
function togglePause() {
  ...
  if (!state.paused) {
    state.monthStartedAt = Date.now();
    if (state.voyage) state.voyage.startedAt = Date.now() - state.voyage.pausedElapsed;
  } else if (state.voyage) {
    state.voyage.pausedElapsed = Date.now() - state.voyage.startedAt;
  }
  render();
}
...
function gameTick() {
  ...
  if (state.voyage && Date.now() - state.voyage.startedAt >= state.voyage.duration) {
    finishVoyage();
  }

  if (Date.now() - state.monthStartedAt >= state.monthDurationMs) {
    nextTurn();
  } else {
    render();
  }
